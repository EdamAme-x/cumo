...
export function HotReload(options?: {
  ...
  return {
    setup(hono: Hono) {
      ...
      const script = `<!-- HOT-RELOAD:${id} --><script cumo-hot-reload>setInterval(()=>{fetch('/_cumo/extensions/hot-reload').then(r=>r.text()).then(i=>{if(i!=='${id}')location.reload()})},${interval + 4})</script>`;
