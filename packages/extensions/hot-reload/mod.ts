 ...
 */
export function HotReload(options?: { interval?: number, dev: boolean }) {
  ...
  return {
    setup(hono: Hono) {
      ...
      const script = `<!-- HOT-RELOAD-ID:${id} --><script cumo-hot-reload>setInterval(()=>{fetch('/_cumo/extensions/hot-reload').then(r=>r.text()).then(i=>{if(i!=='${id}')location.reload()})},${interval})</script>`;
